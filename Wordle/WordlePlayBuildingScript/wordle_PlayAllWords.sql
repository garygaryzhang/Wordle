...
------------------------------------------------------------------------------------------------
------------------------ wordle_GetOptimalNextWord_sample
------------------------------------------------------------------------------------------------
if object_id('wordle_GetOptimalNextWord_sample') is not null
begin
    drop proc wordle_GetOptimalNextWord_sample
end

go

CREATE PROC wordle_GetOptimalNextWord_sample
(	@Player char(25)        
...
_________________________________________________________________________________________________
	...
	proc name: 	wordle_GetOptimalNextWord_sample
	author:		Alberto Martinez
...
--First random try <-- should be in the recursion as well
SELECT Top 1 * 
FROM dbo.WordleWords 
WHERE NumDistinctLetters = 5
ORDER BY NEWID()

SELECT * FROM WordlePlays 

--declare 
--@player varchar(20) = 'Gary', 
--@current_word_date date = '2021-06-30', 
--@guess char(5) = 'EMAIL', 
--@l1 tinyint = 0, 
--@l2 tinyint=0, 
--@l3 tinyint =0, 
...
EXEC dbo.wordle_PlayWord @player,@current_word_date,@guess, @L1 output, @L2 output, @L3 output, @L4 output, @L5 output , @IsDebugRun = 1
...
SET @MaxTryNumber = (SELECT max(TryNumber) FROM dbo.WordlePlays 
				     WHERE Player = @player
			         AND WordDate = @current_word_date)
...
BEGIN
	...
	SELECT @L1Cmd, @L2Cmd, @L3Cmd, @L4Cmd, @L5Cmd

	SET @WhereClausCommand += @L1Cmd + @L2Cmd + @L3Cmd + @L4Cmd + @L5Cmd

	SELECT @WhereClausCommand
...
SELECT TOP 1 @OutPutWord = Word FROM dbo.WordleWords WHERE Word NOT LIKE 'W%' AND Word NOT LIKE '_O%'  AND Word NOT LIKE '__R%'  AND NumS >= 1 AND Word NOT LIKE '___S%'  AND Word NOT LIKE '____E%'  ORDER BY NEWID() 




SELECT * FROM dbo.WordleAlgoResults 

SELECT * FROM dbo.WordleAnswers 

SELECT * FROM dbo.WordlePlays 

SELECT * FROM dbo.WordleWords
WHERE Word NOT LIKE '%O%'
AND Word NOT LIKE '%M%'
AND Word NOT LIKE '%B%'
AND Word NOT LIKE '%R%'
AND Word NOT LIKE '%E%'
...
AND Word NOT LIKE 'P%'
AND NumP >= 1
AND Word NOT LIKE '_I%'
AND NumI >= 1
AND Word LIKE 'S%'
AND NumS >= 1
AND Word LIKE '_P%'
AND Word NOT LIKE '__L%'
AND NumL >= 1
AND Word NOT LIKE '___I%'
AND NumI >= 1
...
FROM dbo.WordleWords
WHERE Word NOT LIKE 'O%'
  AND Word NOT LIKE '_M%'
  AND Word NOT LIKE '__B%'
  AND Word NOT LIKE '___R%'
  AND Word NOT LIKE '____E%'
ORDER BY NEWID()

SELECT * 
FROM dbo.WordleWords
WHERE Word LIKE '_ISER%'


SELECT TOP 1 * FROM #TempDataSetA
ORDER BY NEWID()
...
FROM #TempDataSetA
WHERE Word LIKE 'S%'
AND Word NOT LIKE '_T%'
AND Word NOT LIKE '__O%'
AND Word NOT LIKE '___N%'
AND Word LIKE '____Y%'


SELECT * FROM #TempDataSetB

SELECT TOP 1 * FROM #TempDataSetB
ORDER BY NEWID()

DROP TABLE #TempDataSetA

SELECT *
INTO #TempDataSetA
FROM #TempDataSetB
WHERE Word LIKE 'S%'
AND Word NOT LIKE '_H%'
AND NumA >= 1 
AND Word Like '___D%'
AND Word Like '____Y%'
